defineModule(sim, list(
  name = "CBM_dataPrep_RIApresentDay",
  ...
  version = list(SpaDES.core = "1.0.2", CBM_dataPrep_RIApresentDay = "0.0.1"),
  spatialExtent = raster::extent(rep(NA_real_, 4)),
  timeframe = as.POSIXlt(c(NA, NA)),
  timeunit = "year",
  citation = list("citation.bib"),
  documentation = list("README.txt", "CBM_dataPrep_RIApresentDay.Rmd"),
  reqdPkgs = list(
...
doEvent.CBM_dataPrep_RIApresentDay <- function(sim, eventTime, eventType, debug = FALSE) {
  switch(
    eventType,
    init = {
      ...
      sim <- scheduleEvent(sim, P(sim)$.saveInitialTime, "CBM_dataPrep_RIApresentDay", "save")
    },
    save = {
      # ! ----- EDIT BELOW ----- ! #
      # do stuff for this event
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + P(sim)$.saveInterval, "CBM_dataPrep_RIApresentDay", "save")
